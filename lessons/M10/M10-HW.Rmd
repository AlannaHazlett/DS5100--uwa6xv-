...
# Student Info
    ...
    Name: Alanna Hazlett
    Ned ID: uwa6xv
    URL of this file on GitHub: https://github.com/AlannaHazlett/DS5100--uwa6xv-/tree/main/lessons/M10
...
```{r}
bracket<-c(1,2,3,4)
percent_quota<-c(0.4,0.7,0.9,1)
percent_increment<-c(0.40,0.30,0.20,0.1)
commission<-c(0.07,0.10,0.13,0.16)
quota_payout<-data.frame(bracket,percent_quota,percent_increment,commission)
bracket_value<-c(6300,6750,5850,3600)
payout<-c(6300,13050,18900,22500)
quota_payout<-data.frame(quota_payout,bracket_value,payout)
print(quota_payout)
```
...
```{r}
earned<-function(x,quota_value){
  if(x >= 0 & x<= 0.4) {
    print(x * quota_payout[1,4] * quota_value)
  }
  else if(x > 0.4 & x <= 0.7) {
    print(((x - quota_payout[1,2]) * quota_payout[2,4] * quota_value) + quota_payout[1,6])
  }
  else if (x > 0.7 & x <= 0.9) {
    print(((x - quota_payout[2,2]) * quota_payout[3,4] * quota_value) + quota_payout[2,6])
  }
  else if (x > 0.9 & x <= 1) {
    print(((x - quota_payout[3,2]) * quota_payout[4,4] * quota_value) + quota_payout[3,6])
  }
}
```
...
for (i in seq(from = 0, to=1, by=0.1)) {
  result = data.frame(i,earned(i,225000))
  df2<-rbind(df2,result)
}
names(df2)[c(1,2)]<-c("percent","earned")
print(df2)
```